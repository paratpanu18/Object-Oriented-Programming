...
class Account:
    ...
    def transfer(self, pin, amount, target_account):
        if self.__card.pin == pin:
            self.__amount -= amount
            self.__transaction.append(Transaction("Transfer", amount, self.__amount, target_account.account_no))
            target_account += amount
    
    def paid(self, amount, target_account):
        self.__amount -= amount
        self.__transaction.append(Transaction("EDC", amount, self.__amount, target_account.account_no))
        target_account += amount
...
class Transaction:
    ...
    def __str__(self):
        return f"{self.__transaction_type:<10} {self.__amount:<10}{self.__total:<10}{self.__target_account}"
...
class ATM_machine:
    ...
    def withdraw(self, account, amount):
        if self.__money >= amount and account.amount >= amount:
            account -= amount
            self.__money -= amount
        pass
...
# ! Initial pin ("0000") is incorrect -> So, I change it to ("1234")
harry_account.transfer("1234", 10000, hermione_account)
...
# Test case #5 : ทดสอบการชำระเงินแบบอิเล็กทรอนิกส์ ให้เรียกใช้ method paid จาก tops
...
print("Hermione's Transaction\n=====================")
print(f'{"Type":<10} {"Amount":<10}{"Balance":<10}{"Reciever":<10}')
